...
func RunSymbolSections(db *sql.DB, symbolID int, limit int, priorities map[Priority]bool, budget *RowBudget, selectedChecks map[string]bool, perCheckLimit int) ([]SectionResult, error) {
	checks := []Check{
	...
	return RunChecks(checks, db, limit, priorities, CheckOptions{}, budget, selectedChecks, perCheckLimit)
}
